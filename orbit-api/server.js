...
const attachUser = (req, res, next) => {
	...
	if (!decodedToken) {
		return res.status(401).json({ message: 'There was an authorization problem' });
	}
...
app.patch('/api/user-role', async (req, res) => {
	try {
		...
		res.json({
			message: 'User role updated. You must log in again for the changes to take effect.'
		});
	} catch (err) {
...
app.delete('/api/inventory/:id', checkJWT, requireAdmin, async (req, res) => {
	try {
		const { sub } = req.user;
		const deletedItem = await InventoryItem.findOneAndDelete({ _id: req.params.id, user: sub });
		res.status(201).json({
...
app.get('/api/users', async (req, res) => {
	try {
		const users = await User.find().lean().select('_id firstName lastName avatar bio');
